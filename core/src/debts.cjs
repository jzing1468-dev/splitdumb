"use strict";
function calculateSimplifiedDebts(members, expenses, expenseSplits, settlements) {
  const balances = {};
  ...
  expenseSplits.forEach(s => { balances[s.member_id] = (balances[s.member_id] || 0) - s.share; });
  ...
  while (i < creditors.length && j < debtors.length) {
    ...
    if (amount > 0.01) { transactions.push({ from: debtors[j].id, to: creditors[i].id, amount }); }
  ...
  return {
    balances: Object.fromEntries(Object.entries(balances).map(([id, bal]) => [id, { name: memberMap[id] || 'Unknown', balance: Math.round(bal * 100) / 100 }])),
    transactions: transactions.map(t => ({ from: { id: t.from, name: memberMap[t.from] }, to: { id: t.to, name: memberMap[t.to] }, amount: t.amount }))
  };
}
