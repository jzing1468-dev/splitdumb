// @splitdumb/core — splits.js
// Shared split calculation logic. Pure functions, no DB, no transport.
// Extracted verbatim from server/splitCalc.js.

export const CATEGORIES = ['groceries', 'dining', 'transport', 'accommodation', 'entertainment', 'utilities', 'health'];

export function calculateSplits(type, amount, splitMemberIds, opts = {}) {
  const {
    sharesInput,
    nightsInput,
    nightsMode,
  ...
  if (type === 'equal') {
    ...
    nightLabels.forEach(label => {
      for (const [mid, labels] of Object.entries(nightsInput)) {
        if (splitMemberIds.includes(mid) && labels.includes(label)) {
          occupiedDayCount++;
          break;
        }
      }
    ...
    if (calc === 'daily') {
      ...
      splitMemberIds.forEach(mid => {
        ...
        if (personDays > 0) {
          nightShares[mid] = Math.round(personDays * perOccupiedDay * 100) / 100;
        }
      });
    }
    ...
    if (remainderCents !== 0) {
      const sorted = splitMemberIds
        .map(mid => [mid, (nightsInput[mid] || []).length])
        .sort((a, b) => b[1] - a[1]);
...
/**
 * Build nights_data JSON blob for storage
 */
export function buildNightsData(nightsInput, opts = {}) {
